...
import type { SocketConfig, WASocket } from "../Types/Socket.js";
import type { WAMessageKey, WAMessageSendOptions } from "../Types/Messages.js";
import type { GroupParticipantAction } from "../Types/Groups.js";
...
import type { ConnectionUpdate } from "../Types/Events.js";
import {
  buildChatstateNode,
  buildPresenceNode,
} from "../Protocol/chatstate.js";

function resolveLogger(config: SocketConfig): Logger {
  ...
  connection.setPayloadHandler?.((payload) => {
    ...
    messageEngine.handlePayload(
      ...
      session
        ? {
            session,
            sendFrame: (frame: Buffer) => connection.transport.send(frame),
          }
        : undefined,
    );
  });
  ...
  const sock: WASocket = {
    ...
    async sendMessage(jid, content, options) {
      return messageEngine.sendMessage(
        jid,
        content,
        { ...(options as WAMessageSendOptions), userJid: state.user?.id },
        optionalNet(),
      );
    },

    async sendReaction(key: WAMessageKey, emoji: string) {
      return messageEngine.sendReaction(key, emoji, optionalNet());
    },

    async sendRevoke(key: WAMessageKey) {
      return messageEngine.sendRevoke(key, optionalNet());
    },

    async sendEdit(key: WAMessageKey, text: string) {
    ...
    async requestPairingCode(phoneNumber: string, timeoutMs = 60_000) {
    ...
    groupCreate: (subject, participants) =>
      Groups.groupCreate(subject, participants, requireNet()),
    groupMetadata: (jid) => Groups.groupMetadata(jid, requireNet()),
    groupParticipantsUpdate: (jid, participants, action: GroupParticipantAction) =>
      Groups.groupParticipantsUpdate(jid, participants, action, requireNet()),
    groupUpdateSubject: (jid, subject) =>
      Groups.groupUpdateSubject(jid, subject, requireNet()),
    groupUpdateDescription: (jid, description) =>
      Groups.groupUpdateDescription(jid, description, requireNet()),
    ...
    async waitForConnectionUpdate(check, timeoutMs = 60_000) {
      return promiseTimeout(
        ...
        new Promise<void>((resolve, reject) => {
          const listener = (update: ConnectionUpdate) => {
            try {
              if (check(update)) {
