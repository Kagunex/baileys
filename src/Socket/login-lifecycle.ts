...
import { parsePairingPayload, type PairingResult } from "../Protocol/pairing.js";
...
export function resolveLoginMode(creds?: AuthenticationCreds): LoginMode {
 ...
 */
export function detectDisconnectFromPayload(payload: Buffer): DisconnectReason | undefined {
 ...
 */
export function applyPairSuccess(
  pairing: PairingResult,
  existing?: AuthenticationCreds,
): PairSuccessApply | undefined {
  // Explicit pair-success is mandatory — do not treat "me" alone as success
  if (pairing.pairSuccess !== true) return undefined;

  const me = pairing.me;
  // Device identity from the pair-success frame is required
  if (!me?.id || typeof me.id !== "string") return undefined;
  // Minimal JID shape: user@server (e.g. 628xxx@s.whatsapp.net)
  if (!me.id.includes("@") || me.id.length < 5) return undefined;
  ...
  if (existing) {
    const hasNoise =
      existing.noiseKey?.public?.length && existing.noiseKey?.private?.length;
    const hasIdentity =
      existing.signedIdentityKey?.public?.length &&
      existing.signedIdentityKey?.private?.length;
    if (!hasNoise || !hasIdentity) return undefined;
  }
 ...
 */
export function buildQrFromServerRef(
  ref: string,
 ...
 */
export function shouldSkipPairingOnReconnect(creds?: AuthenticationCreds): boolean {
 ...
 */
export function applyLoggedOut(creds: AuthenticationCreds): Partial<AuthenticationCreds> {
  return {
    registered: false,
    me: undefined,
    pairingCode: undefined,
