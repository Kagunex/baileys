...
export function createConnectionController(
  ...
  const requestPairingCode = (phoneNumber: string, timeoutMs = 60_000): Promise<string> => {
    ...
    // Socket readiness: Noise handshake + client payload must be done
    if (!session || (phase !== "login_sent" && phase !== "noise_done")) {
      return Promise.reject(
        new Error(
          "WAITING_FOR_SOCKET_READY: requestPairingCode requires completed Noise handshake + client payload",
        ),
      );
    }
