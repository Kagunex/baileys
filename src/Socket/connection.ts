...
export function createConnectionController(
  ...
  /** Waiters for Noise + client-payload readiness (pairing prerequisite) */
  type ReadyWaiter = {
    resolve: () => void;
  ...
  const isPairingReady = (): boolean =>
    !!session && (phase === "login_sent" || phase === "noise_done");

  const settleReadyWaiters = (err?: Error) => {
  ...
  const waitForPairingReady = (timeoutMs = 60_000): Promise<void> => {
  ...
  const handleDecrypted = (pt: Buffer) => {
...

    {
      ...
      // Only persist pairingCode when the response IQ id matches the
      // currently active pairing attempt. isBusy() alone is insufficient:
      // a stale / wrong / unsolicited code-bearing frame must never mutate
      // auth state or emit creds.update.
      if (
        pairingParsed.code &&
        state.auth &&
        pairing.isActiveIq(pairingParsed.iqId)
      ) {
    ...
    const authReady =
      !!state.auth?.creds.registered && !!state.auth?.creds.me?.id;
