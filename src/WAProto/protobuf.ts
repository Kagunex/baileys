...
export function fieldBytes(fields: ProtoField[], number: number): Buffer | undefined {
  ...
  if (!f || typeof f.value === "bigint") return undefined;
  if (Buffer.isBuffer(f.value)) return f.value;
  if ((f.value as any) instanceof Uint8Array) return Buffer.from(f.value);
  return undefined;
}
...
/** Encode a uint64 field using varint (for phone numbers > 2^32). */
export function encodeVarint64(fieldNumber: number, value: bigint): Buffer {
  const tag = writeVarint((fieldNumber << 3) | 0);
  const out: number[] = [];
