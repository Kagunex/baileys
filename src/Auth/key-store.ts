...
function keyFilePath(folder: string, type: string, id: string) {
...
function reviveValue(value: unknown): unknown {
  if (value && typeof value === "object" && (value as { __type?: string }).__type === "buffer") {
    return new Uint8Array(
      decodeBase64((value as { data: string }).data),
    );
  }
...
export function makeCacheableSignalKeyStore(folder: string): SignalKeyStore {
  ...
  async function recoverJournal() {
    try {
      ...
      for (const e of entries) {
        try {
          if (e.op === "delete") await fs.unlink(e.file).catch(() => undefined);
  ...
  return {
      ...
      await Promise.all(
        ids.map(async (id) => {
          try {
            ...
            result[id] = reviveValue(JSON.parse(raw)) as SignalDataTypeMap[typeof type];
          } catch {
            /* missing */
          }
        }),
    ...
    async set(data) {
      return mutex(async () => {
        await ensureDir(folder);
        const journal: Array<{ file: string; op: "write" | "delete"; payload?: string }> =
          [];
        ...
        for (const type of Object.keys(data) as Array<keyof SignalDataSet>) {
          ...
          for (const id of Object.keys(entries)) {
            ...
            const file = keyFilePath(folder, type as string, id);
            if (value == null) {
