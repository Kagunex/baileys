...
function readListSize(tag: number, reader: BinaryReader): number {
...
function readStringFromToken(tag: number, reader: BinaryReader): string {
  ...
  if (tag === TAGS.NIBBLE_8 || tag === TAGS.HEX_8) {
    ...
    for (let i = 0; i < raw.length; i++) {
      ...
      if (i * 2 + 1 < size && low !== 0x0f) out += hexChars[low];
    }
    return out;
  }
...
function readBinary(tag: number, reader: BinaryReader): Buffer {
  let len: number;
