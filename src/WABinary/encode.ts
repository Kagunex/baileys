...
function pushByte(out: number[], value: number) {
  out.push(value & 0xff);
}

function pushInt20(out: number[], value: number) {
...
function writeBinary(out: number[], data: Buffer | Uint8Array) {
...
function writeJid(out: number[], jid: string) {
  const at = jid.indexOf("@");
  if (at > 0) {
    const user = jid.slice(0, at);
    const server = jid.slice(at + 1);
    pushByte(out, TAGS.JID_PAIR);
    if (user.length === 0) pushByte(out, TAGS.LIST_EMPTY);
...
function writeListStart(out: number[], count: number) {
  if (count === 0) {
...
function writeNode(out: number[], node: BinaryNode) {
