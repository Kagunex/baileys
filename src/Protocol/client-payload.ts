...
function encodeAppVersion(v: [number, number, number]): Buffer {
...
function encodeWebInfo(): Buffer {
  return encodeVarint(3, WEB_BROWSER); // webSubPlatform = WEB_BROWSER
}

function encodeCompanionProps(version: [number, number, number]): Buffer {
  return Buffer.concat([
...
function encodeDevicePairingData(version: [number, number, number]): Buffer {
 ...
 */
export function encodeClientPayload(options: ClientPayloadOptions = {}): Buffer {
