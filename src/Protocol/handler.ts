...
function collectQrRefs(node: BinaryNode, out: string[]): void {
...
export function parseProtocolPayload(payload: Buffer): ProtocolParseResult {
  const qrRefs: string[] = [];
...
export function composeQrPayload(parts: {
  ...
  if (!parts.ref || parts.ref.length < 8) return undefined;
  if (!parts.noisePub?.length || !parts.identityPub?.length) return undefined;
  if (!parts.advSecretKey) return undefined;
