...
function generateRegistrationId() {
  return (0, import_node_crypto3.randomBytes)(2).readUInt16BE(0) & 16383;
}
function generateMessageID(prefix = "") {
  const id = (0, import_node_crypto3.randomBytes)(8).toString("hex").toUpperCase();
...
function generateX25519KeyPair2() {
  const { publicKey, privateKey } = (0, import_node_crypto4.generateKeyPairSync)("x25519");
  const pubDer = publicKey.export({ type: "spki", format: "der" });
  const privDer = privateKey.export({ type: "pkcs8", format: "der" });
  return {
...
function hkdf(ikm, length, info = "", salt) {
  ...
  const prk = (0, import_node_crypto4.createHmac)("sha256", saltBuf).update(Buffer.from(ikm)).digest();
  const blocks = Math.ceil(length / 32);
  let t = Buffer.alloc(0);
  const okm = Buffer.alloc(blocks * 32);
  for (let i = 0; i < blocks; i++) {
    t = (0, import_node_crypto4.createHmac)("sha256", prk).update(t).update(infoBuf).update(Buffer.from([i + 1])).digest();
    t.copy(okm, i * 32);
  }
  return Buffer.from(Uint8Array.from(okm.subarray(0, length)));
}
function aesEncryptGCM(plaintext, key, iv, additionalData) {
  const cipher = (0, import_node_crypto4.createCipheriv)("aes-256-gcm", Buffer.from(key).subarray(0, 32), Buffer.from(iv));
...
function aesDecryptGCM(ciphertextWithTag, key, iv, additionalData) {
  ...
  const decipher = (0, import_node_crypto4.createDecipheriv)("aes-256-gcm", Buffer.from(key).subarray(0, 32), Buffer.from(iv));
...
function initAuthCreds() {
  const signedPreKey = {
    ...
    signature: new Uint8Array((0, import_node_crypto5.randomBytes)(64)),
    keyId: 1,
  ...
  return {
    ...
    advSecretKey: encodeBase64((0, import_node_crypto5.randomBytes)(32)),
...
function x25519Agree(privateKey, publicKey) {
  const pkcs8Prefix = Buffer.from("302e020100300506032b656e04220420", "hex");
  const spkiPrefix = Buffer.from("302a300506032b656e032100", "hex");
  const priv = (0, import_node_crypto6.createPrivateKey)({
    key: Buffer.concat([pkcs8Prefix, privateKey]),
    format: "der",
    type: "pkcs8"
  });
  const pub = (0, import_node_crypto6.createPublicKey)({
    key: Buffer.concat([spkiPrefix, publicKey]),
    format: "der",
    type: "spki"
  });
  return (0, import_node_crypto6.diffieHellman)({ privateKey: priv, publicKey: pub });
}
...
function signPreKey(identity, preKeyPublic) {
  const h = (0, import_node_crypto7.createHmac)("sha512", Buffer.from(identity.private));
