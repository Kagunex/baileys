...
export function parseGroupMetadata(node) {
    // iq > group | iq > groups > group
    let group = getBinaryNodeChild(node, "group") ||
    ...
    if (!group)
        return undefined;
    const id = getBinaryNodeAttr(group, "id") ||
    ...
    const desc = (descNode && typeof descNode.content === "string" && descNode.content) ||
...
export function parseGroupCreateResult(node) {
    return parseGroupMetadata(node);
}
export function parseGroupModification(node) {
    const participants = [];
...
export function parseInviteCode(node) {
    const invite = getBinaryNodeChild(node, "invite") ||
        getBinaryNodeChild(getBinaryNodeChild(node, "group") || node, "invite");
    if (!invite)
        return getBinaryNodeAttr(node, "code");
