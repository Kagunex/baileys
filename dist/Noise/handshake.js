...
/**
 * Noise spec §3.4 — initialize handshake state.
 *
 * FIXED: Per spec, if len(protocolName) <= HASHLEN, set h = protocolName || zeros
 * (zero-padded to HASHLEN).  Only hash if len > HASHLEN.
 *
 * RC1 bug: used sha256(28-byte name) instead of zero-padding to 32 bytes.
 * WhatsApp uses "Noise_XX_25519_AESGCM_SHA256\0\0\0\0" (32 bytes) as the
 * protocol name, which under the zero-pad rule becomes h0 directly — no SHA256.
 * The SHA256 path produced a completely different h0 and ck, causing all
 * subsequent AES-GCM key material to mismatch the server → authentication failure.
 */
function initializeHash(prologue) {
    const name = Buffer.from(NOISE_PROTOCOL_NAME, "utf-8"); // 28 bytes
    // Noise spec §3.4: len(name) <= HASHLEN → zero-pad; len(name) > HASHLEN → HASH(name)
    // "Noise_XX_25519_AESGCM_SHA256" is 28 bytes < 32 = HASHLEN → zero-pad.
    // Zero-padded result equals Buffer.from(NOISE_MODE, "binary") which WhatsApp
    // uses as h0 directly.  Do NOT sha256() — that path is for names > 32 bytes.
    let h0;
    if (name.length <= HASH_LEN) {
        h0 = Buffer.alloc(HASH_LEN, 0);
        name.copy(h0);
    }
    else {
        h0 = sha256(name);
