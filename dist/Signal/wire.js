...
export function encodeSignalWire(message) {
    const pub = message.ratchetPub;
    if (pub.length !== 32)
        throw new Error("ratchetPub must be 32 bytes");
...
export function decodeSignalWire(buf) {
...
export function unwrapEncryptedBody(buf) {
    if (buf.length < 2)
