...
export function encryptMedia(plaintext) {
...
export function decryptMedia(ciphertextWithMac, mediaKey) {
    const data = toBuffer(ciphertextWithMac);
    if (data.length < 10)
        throw new MediaError("Ciphertext too short");
    ...
    try {
        ...
        throw new MediaError("Media decryption failed", { cause: err });
    }
}
