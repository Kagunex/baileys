 ...
 */
export function buildPairingCodeIq(phoneNumber, options = {}) {
    ...
    const regChildren = [
        keyChild("link_code_pairing_wrapped_companion_ephemeral_pub", keys.companionEphemeralPub),
...
export function buildPairDeviceIq(ref, companionRef) {
    return {
        tag: "iq",
        attrs: {
            to: "s.whatsapp.net",
...
function walkPairing(node, out) {
    ...
    if (node.tag === "pair-success") {
        ...
        const jid = (device && getBinaryNodeAttr(device, "jid")) ||
            getBinaryNodeAttr(node, "jid");
        if (jid) {
            out.me = {
                id: jid,
                name: getBinaryNodeAttr(node, "name") ||
...
export function extractPairingCode(payload) {
 ...
 */
export function isPairingResponse(payload, expectedId, opts) {
    try {
        const node = decodeBinaryNode(payload);
        const parsed = parsePairingPayload(payload);
        if (parsed.pairSuccess)
            return true;
        if (node.tag === "iq") {
            const id = getBinaryNodeAttr(node, "id");
        ...
        if (opts?.acceptUnsolicitedCode && parsed.code)
            return true;
        return false;
    }
    catch {
...
export function isPairingIqResult(payload, expectedId) {
    return isPairingResponse(payload, expectedId);
}
/** Retry policy helper */
export function pairingRetryDelayMs(attempt) {
